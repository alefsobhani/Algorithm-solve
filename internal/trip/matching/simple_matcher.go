...
type ReservationStore interface {
	TryReserve(ctx context.Context, driverID uuid.UUID, tripID uuid.UUID) (bool, error)
}
...
func NewSimpleMatcher(source CandidateSource, store ReservationStore, limit int) *SimpleMatcher {
	...
	return &SimpleMatcher{source: source, store: store, limit: limit}
}
...
func (m *SimpleMatcher) ReserveDriver(ctx context.Context, trip domain.Trip) (*uuid.UUID, error) {
	...
	for _, driverID := range candidates {
		reserved, err := m.store.TryReserve(ctx, driverID, trip.ID)
		if err != nil {
			return nil, err
		}
		if reserved {
...
func (m *MemoryReservationStore) TryReserve(_ context.Context, driverID uuid.UUID, tripID uuid.UUID) (bool, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
